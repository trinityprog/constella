...
@section('content_header')
    ...
    <h1 class="text-center mb-4">Изменить адрес #{{ $address->id }}</h1>
@stop
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.contact-addresses.update', $address->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                        {{ method_field('PATCH') }}
                        {{ csrf_field() }}
                        <input type="hidden" name="city_id" value="{{ $city->id }}">
                        @include ('admin.contact-addresses.form', ['formMode' => 'edit'])
                    </form>
                </div>
            </div>
        </div>
