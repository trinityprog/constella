<template>
  <div>
    ...
    <div class="vizard__pretext">Выделить камень в сердце <Hint title="Выделить камень в сердце" text="Вы можете выделить сердце на фигурке, вставив на его место камень" pos="right" /></div>
...
export default {
  ...
  computed: {
    ...
    stepData: {
      get() {
        return DATA.find(el => el.id == this.currentOrder.step2Value).step62[this.currentOrder.step3Value][this.currentOrder.step61Value].map(el => ({
          ...el,
          active: el.id == this.currentOrder.step62Value,
        }));
      },
      set() {
      },
  ...
  mounted() {
    this.nextStep = DATA.find(el => el.id == this.currentOrder.step2Value).step62.next;
    this.prevStep = DATA.find(el => el.id == this.currentOrder.step2Value).step62.prev;
  },
  data() {
    return {
      nextStep: 0,
  ...
  methods: {
    ...mapMutations('figures', ['setCurrentStep', 'setStep62Value', 'setCurrentImage']),
    setActive(id) {
      this.setStep62Value(id);
