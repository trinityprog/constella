...
export default {
  ...
  computed: {
    ...
    stepData: {
      get() {
        return DATA.find(el => el.id == this.currentOrder.step2Value).step51[this.currentOrder.step3Value][this.currentOrder.step5Value].map(el => ({
          ...el,
          active: el.id == this.currentOrder.step51Value,
        }));
      },
      set() {
      },
  ...
  methods: {
    ...mapMutations('figures', ['setCurrentStep', 'setStep62Value', 'setCurrentImage', 'setStep51Value']),
    setActive(id) {
      this.setStep51Value(id);
      const image = `/assets/step5_1/${this.currentOrder.step2Value}/${this.currentOrder.step5Value}/image_${this.currentOrder.step51Value}_${this.currentOrder.step3Value}.png`;
