<template>
  <div>
    ...
    <div class="vizard__pretext">Цвет эмали <Hint title="Цвет эмали" text="Выберите цвет эмали" pos="right" /></div>
...
export default {
  ...
  computed: {
    ...
    stepData: {
      get() {
        return DATA.find(el => el.id == this.currentOrder.step2Value).step6[this.currentOrder.step3Value].map(el => ({
          ...el,
          active: el.id == this.currentOrder.step61Value,
        }));
      },
      set() {
      },
  ...
  mounted() {
    this.nextStep = DATA.find(el => el.id == this.currentOrder.step2Value).step6.next;
    this.prevStep = DATA.find(el => el.id == this.currentOrder.step2Value).step6.prev;
  },
  data() {
    return {
      nextStep: 0,
  ...
  methods: {
    ...mapMutations('figures', ['setCurrentStep', 'setStep61Value', 'setCurrentImage']),
    setActive(id) {
      this.setStep61Value(id);
